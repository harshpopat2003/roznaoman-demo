  ...
  return (
    <Room id={plan[5].id} state={state} vertical={vertical} width={112} className="grain bg-ink">
      ...
      <div className="relative mx-auto flex w-screen max-w-[100vw] min-h-svh flex-col h-full px-6 pb-28 pt-36 sm:px-10 md:pb-16 lg:px-16 lg:pt-28">
        ...
        <div className="mt-14 flex min-h-0 flex-1 items-center">
          <div className="grid w-full gap-8 sm:grid-cols-3 lg:gap-12">
            {methods.map((m, i) => (
              <article key={m.n} data-enter={String(Math.min(5, i + 3))} className="group lg:mx-auto">
                {/* Width follows from the height, not the column, below
                    `lg` — a full-bleed 4:5 column on a wide, short desktop
                    screen grows taller than the room has to give it. */}
                <div className="arch relative aspect-[4/5] w-full overflow-hidden bg-night lg:h-[38vh] lg:w-auto">
                  <Image
                    src={m.img}
                    alt={m.title}
                    fill
